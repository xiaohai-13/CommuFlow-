...
KNOWLEDGE_BASE = [
    ...
    {
        "source": "《质检标准》第3.2条",
        "content": "产品出库前须通过质检抽检，AQL标准为1.0。不合格批次需全部退回生产线，由生产部填写《不合格品处理单》，品质部跟进复检。"
    },
    {
        "source": "《跨部门协作规范》FAQ",
...
def search_knowledge(question: str, top_k: int = 3) -> list[str]:
    ...
    for doc in KNOWLEDGE_BASE:
        ...
        keywords = set(question_lower.split()) & set(content_lower.split())
        score = len(keywords) / max(len(question_lower.split()), 1)
